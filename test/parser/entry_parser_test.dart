...
void main() {
  group('EntryParser - 花费解析测试', () {
    test('解析 买菜 35.5 - 识别为花费', () {
      ...
      expect(entry.expense!.amount, equals(35.5));
    ...
    test('解析 打车 ¥30元 - 识别为花费', () {
      ...
      expect(entry.type, equals(NoteEntryType.expense));
      expect(entry.expense, isNotNull);
      expect(entry.expense!.amount, equals(30.0));
    ...
    test('解析 ¥100 购物 - 识别为花费', () {
      ...
      expect(entry.type, equals(NoteEntryType.expense));
      expect(entry.expense, isNotNull);
      expect(entry.expense!.amount, equals(100.0));
    ...
    test('解析 加油 200元 - 识别为花费', () {
      ...
      expect(entry.type, equals(NoteEntryType.expense));
      expect(entry.expense!.amount, equals(200.0));
      expect(entry.expense!.category, equals('交通'));
    });
    ...
    test('解析 买药花了50元 - 识别为花费', () {
      ...
      expect(entry.type, equals(NoteEntryType.expense));
      expect(entry.expense!.amount, equals(50.0));
    ...
    test('解析 10块5毛 - 识别为花费', () {
      ...
      expect(entry.type, equals(NoteEntryType.expense));
      expect(entry.expense!.amount, equals(10.5));
    });
    ...
    test('解析 3块2 - 识别为花费', () {
      ...
      expect(entry.type, equals(NoteEntryType.expense));
      expect(entry.expense!.amount, equals(3.2));
    });
  ...
  group('EntryParser - 事项解析测试', () {
    ...
    test('解析 记得买牛奶 - 不带日期关键词，识别为备忘', () {
      final entry = EntryParser.parse('记得买牛奶');
      
      expect(entry.type, equals(NoteEntryType.memo));
      expect(entry.memoText, equals('记得买牛奶'));
    });
  });
...
天气很好''';
      ...
      expect(entries[0].expense!.amount, equals(35.5));
...
明天去医院''';
  ...
  group('EntryParser - 边界情况测试', () {
    test('带小数点的金额 - 0.5元', () {
      ...
      expect(entry.type, equals(NoteEntryType.expense));
      expect(entry.expense!.amount, equals(0.5));
    });
    ...
    test('大金额 - 10000元', () {
      ...
      expect(entry.type, equals(NoteEntryType.expense));
      expect(entry.expense!.amount, equals(10000.0));
    });

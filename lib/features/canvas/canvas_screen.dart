...
import 'widgets/ai_preview_sheet.dart';
...
class _CanvasScreenState extends State<CanvasScreen> {
  ...
  Future<void> _initOcrEngine() async {
    try {
      ...
      ErrorLog.instance.warn('canvas.ocr.init', 'OCR 引擎初始化失败',
          error: e, stack: st);
      if (mounted) {
        setState(() {
          _ocrEngine = null;
  ...
  Future<void> _performAutosave() async {
    ...
    try {
      ...
      ErrorLog.instance.warn('canvas.autosave', '自动保存失败',
          error: e, stack: st);
    } finally {
  ...
  Future<void> _previewAiExtraction() async {
    ...
    try {
      ...
      if (!result.success) {
        ...
        ErrorLog.instance.warn('canvas.ai_preview',
            'AI 预览失败，将回退本地规则',
            error: aiFailureMessage);
      }
    } catch (error, st) {
      aiFailureMessage = error.toString();
      ErrorLog.instance.error('canvas.ai_preview',
          'AI 预览异常，将回退本地规则',
          error: error, stack: st);
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (sheetContext) {
        return Padding(
          ...
          child: SizedBox(
            height: sheetHeight,
            child: AiPreviewSheet(result: result),
          ),
        );
      },
    );
  ...
  void _onScaleUpdate(ScaleUpdateDetails details, CanvasState state) {
    ...
    if (_isDrawing) {
      if (state.stylusOnlyMode && !_stylusActive) return;
      final worldPoint = _screenToWorld(details.localFocalPoint);
      // 热路径：直接 add 到可变 List，避免每帧 spread 整条笔迹。
      // CanvasPainter 端通过 UnmodifiableListView 拿到不同 identity 触发重绘。
      setState(() {
        _currentPoints.add(worldPoint);
        _currentPressures.add(_lastPointerPressure);
      });
      return;
    }
              ...
              child: AnimatedBuilder(
                ...
                builder: (context, _) => CustomPaint(
                  painter: CanvasPainter(
                    ...
                    currentPoints: UnmodifiableListView(_currentPoints),
                    currentPressures:
                        UnmodifiableListView(_currentPressures),
  ...
  Future<void> _saveNote() async {
    ...
    try {
      ...
      ErrorLog.instance.error('canvas.save', '用户保存失败',
          error: e, stack: st);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
